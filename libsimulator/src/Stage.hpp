        ...
        candidates.erase(
            std::remove_if(
                ...
                [&slot_pos, &boundary](const auto& neighbor) {
                    const auto agent_to_neighbor = LineSegment(slot_pos, Pos(neighbor));
                    if(std::find_if(
                           boundary.cbegin(),
                           boundary.cend(),
        ...
        for(const auto& agent : candidates) {
            if(agent.stageId == id) {
                if(std::find(std::begin(occupants), std::end(occupants), agent.id) ==
                   std::end(occupants)) {
                    const auto distance = (Pos(agent) - slots[index]).Norm();
...
public:
    ...
    Point Target(const GenericAgent& agent) override { return agent.routing.target; };
