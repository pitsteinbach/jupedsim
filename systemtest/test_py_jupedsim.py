    ...
    for new_pos, id in zip(
        ...
        expected_agent_ids.append(
            simulation.add_agent(
                journey_id,
                exit,
                jps.CollisionFreeSpeedModelState(position=new_pos),
            )
        )
...
def test_can_run_simulation():
    ...
    simulation = jps.Simulation(
        model=jps.ModelType.COLLISION_FREE_SPEED, geometry=p1.union(p2)
    )
    ...
    agent_id = simulation.add_agent(
        journey_id,
        exit_stage_id,
        jps.CollisionFreeSpeedModelState(position=(6, 6)),
    )
...
def test_can_wait():
    ...
    agent_id = simulation.add_agent(
        journey_id,
        wp,
        jps.CollisionFreeSpeedModelState(position=(30, 30)),
    )
...
def test_can_change_journey_while_waiting():
    ...
    simulation.add_agent(
        journeys[0],
        wp,
        jps.CollisionFreeSpeedModelState(position=(10, 50)),
    )

    simulation.add_agent(
        journeys[0],
        wp,
        jps.CollisionFreeSpeedModelState(position=(8, 50)),
    )

    simulation.add_agent(
        journeys[0],
        wp,
        jps.CollisionFreeSpeedModelState(position=(6, 50)),
    )
...
def test_get_agent_non_existing_agent_from_simulation():
    ...
    agent_id = simulation.add_agent(
        journey_id,
        exit_id,
        jps.CollisionFreeSpeedModelState(position=(7, 7)),
    )
...
def test_agent_can_not_be_added_outside_geometry():
    ...
    simulation.add_agent(
        journey_id,
        exit_id,
        jps.CollisionFreeSpeedModelState(position=agent_position),
    )
    ...
    with pytest.raises(
        ...
        assert simulation.add_agent(
            journey_id,
            exit_id,
            jps.CollisionFreeSpeedModelState(position=(-50, -50)),
        )
