...
def test_collision_free_speed_model_can_not_set_radius_zero(
    ...
    with pytest.raises(
        ...
        simulation.add_agent(
            journey_id,
            exit_id,
            jps.CollisionFreeSpeedModelState(position=agent_position, radius=0),
        )
...
def test_add_agent_rejects_state_of_different_model_on_custom_simulation():
    ...
    with pytest.raises(
        ...
        simulation.add_agent(
            journey_id, exit_id, jps.SocialForceModelState(position=(0, 0))
        )
...
def test_add_agent_rejects_custom_state_on_builtin_model_simulation():
    ...
    with pytest.raises(
        ...
        simulation.add_agent(
            journey_id, exit_id, _MinimalCustomState(position=(0, 0))
        )
...
def test_add_agent_rejects_object_without_position():
    ...
    with pytest.raises(TypeError, match="CustomModelAgentState"):
        simulation.add_agent(journey_id, exit_id, object())
    ...
    def compute_new_position(self, dt, ped, geometry, neighborhood_search):
        try:
            self.simulation.add_agent(
                self.journey_id,
                self.stage_id,
                _MinimalCustomState(position=(5, 5)),
            )
