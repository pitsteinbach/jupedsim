...
@pytest.mark.parametrize(
    "model, state_class, extra_state_kwargs, removed_attrs", model_params
)
def test_desired_speed_can_be_set_via_state(
    create_simulation, model, state_class, extra_state_kwargs, removed_attrs
):
    ...
    agent_id = sim.add_agent(
        journey_id,
        wp,
        state_class(position=(1, 1), desired_speed=1.5, **extra_state_kwargs),
    )
...
def test_desired_speed_can_be_mutated_via_agent_handle(
    ...
    agent_id = sim.add_agent(
        journey_id,
        wp,
        state_class(position=(1, 1), **extra_state_kwargs),
    )
