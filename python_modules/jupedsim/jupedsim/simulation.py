...
_MODEL_FACTORIES = {
    ModelType.COLLISION_FREE_SPEED: py_jps.CollisionFreeSpeedModel,
    ModelType.COLLISION_FREE_SPEED_V2: py_jps.CollisionFreeSpeedModelV2,
    ModelType.COLLISION_FREE_SPEED_V3: py_jps.CollisionFreeSpeedModelV3,
        ...
        **kwargs: Any,
        ...
        Arguments:
            model:
                ...
                Stateless built-in models are selected with a
                :class:`~jupedsim.models.ModelType` member, e.g.
                ``ModelType.COLLISION_FREE_SPEED``. Models with
                simulation-global state are passed as instances:
                :class:`~jupedsim.AnticipationVelocityModel` or
                :class:`~jupedsim.WarpDriverModel`. Custom Python models are
                passed as instances of a
                :class:`~jupedsim.CustomOperationalModel` subclass.

                .. warning::

                    ``AnticipationVelocityModel`` and ``WarpDriverModel``
                    instances are consumed by this constructor and must not be
                    reused afterwards.
            geometry:
        ...
        if isinstance(model, ModelType):
            py_jps_model = _MODEL_FACTORIES[model]()
        elif isinstance(model, py_jps.OperationalModel):
            py_jps_model = model
        elif isinstance(model, CustomOperationalModel):
            py_jps_model = py_jps._PythonModel(model)
        else:
            raise TypeError(
                "model must be a ModelType member, an "
                "AnticipationVelocityModel or WarpDriverModel instance, or a "
